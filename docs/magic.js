...
`}),o({cx:"192",cy:"128",r:"32"}),o({cx:"128",cy:"256",r:"32"}),o({cx:"288",cy:"384",r:"32"}),o({cx:"272",cy:"272",r:"16"}),o({cx:"400",cy:"336",r:"16"}),o({cx:"176",cy:"368",r:"16"})])]))},D=({page:i,state:m},e)=>{let a={id:"Magic",class:m.pageClass};return w(a,c({class:{Wrapper:!0}},[A(m),c({class:"Page",id:"page"},i(m)),M(m),e]))},R=(m,e)=>{"string"==typeof m?m={content:m}:e?m=i({content:e},m):Array.isArray(m)&&(m={content:m.join("")});let{content:a,lines:p=!0}=m;return c({class:{Pre:!0,lines:p&&"false"!==p}},[c({class:"menu"},[n({onclick:[I.pre.clip,i=>({e:i,content:a})]},"copy")]),z(a.trim().split("\n").map(R.Line))])};R.Comment=i=>q({class:"comment"},i),R.Line=i=>l({class:"line"},R.Words(i)),R.Word=i=>{if(!i)return"";let m=i.includes("://"),e=i.startsWith("mailto:")||i.includes("@")&&i.includes(".");if(m||e)return W({to:i,text:i});let a="";return("state"===i?a="state":"actions"===i?a="actions":"effects"===i?a="effects":"subscriptions"===i?a="subscriptions":F.pre.keywords.includes(i)?a="keyword":F.pre.builtins.includes(i)?a="builtin":F.pre.booleans.includes(i)&&(a="boolean"),a)?q({class:a},i):i},R.Words=i=>{let[m,...e]=i.split(F.pre.commentRegex),a=!m.endsWith(":")&&e.length;if(a)return[R.Words(m),R.Comment(e.join("").split(F.pre.wordRegex).map(R.Word))];let p=[],s=i;if(i.replace(F.pre.stringRegex,i=>{if(s){let[m,e]=s.split(i);m&&p.push(m.split(F.pre.wordRegex).map(R.Word).filter(i=>i)),s=e}p.push(q({class:"string"},i))}),s!==i)return s&&p.push(s.split(F.pre.wordRegex).map(R.Word).filter(i=>i)),p;let t=i.split(F.pre.wordRegex).filter(i=>i);return t.map(R.Word)};let F={pre:{booleans:["true","false"],builtins:["Array","Object","String","Number","RegExp","Null","Symbol","Set","WeakSet","Map","WeakMap","setInterval","setTimeout","Promise","JSON","Int8Array","Uint8Array","Uint8ClampedArray","Int16Array","Uint16Array","Int32Array","Uint32Array","Float32Array","Float64Array"],commentRegex:/(\/\/)/gim,keywords:["let","this","long","package","float","goto","private","class","if","short","while","protected","with","debugger","case","continue","volatile","interface","instanceof","super","synchronized","throw","extends","final","export","throws","try","import","double","enum","boolean","abstract","function","implements","typeof","transient","break","default","do","static","void","int","new","async","native","switch","else","delete","null","public","var","await","byte","finally","catch","in","return","for","get","const","char","module","exports","require","npm","install","=>"],stringRegex:/("|')(.*?)\1/gim,wordRegex:/( )/gim},preventDefault:i=>(i.preventDefault(),i)},I={changeTheme:e=>m(i({},e),{pageClass:m(i({},e.pageClass),{light:"dark"===e.theme}),theme:"dark"===e.theme?"light":"dark"}),go:(e,a)=>{let p=a.currentTarget.href.replace(window.location.origin,""),[s,t=""]=p.split("#");if(s===e.url&&t===e.hash)return t&&(window.location.hash=t),e;let n=e.pages&&e.pages[s]&&e.pages[s].title;n&&(document.title=e.title=n),s!==e.url?t||window.scrollTo({top:0}):window.location.hash=t;let{scrollY:o}=window;return window.history.pushState({url:s,hash:t,scrollY:o},e.title,p),m(i({},e),{url:s,hash:t,prev:e.url})},nospy:{toggle:m=>(m.nospy.show=!m.nospy.show,i({},m))},pop:(e,a)=>{let{pathname:p,hash:s}=window.location;s=s.substring(1);let t=0;return a.state&&(p=a.state.url,s=a.state.hash,t=a.state.scrollY||0),s?window.location.hash=s:window.scroll({top:t}),m(i({},e),{url:p,hash:s})},pre:{clip:(i,{content:m})=>{if("undefined"!=typeof document&&"function"==typeof document.execCommand){let i=document.createElement("textarea");i.id="copy",i.innerHTML=m,document.body.appendChild(i);let e=document.getElementById("copy");e.select(),document.execCommand("copy"),document.body.removeChild(e)}return i}}},G={listenPopState:(i,m)=>{let e=e=>i(m,e);return addEventListener("popstate",e),()=>removeEventListener("popstate",e)}},_={"/mime-types/":i=>{var m;return[v(i.title),null===(m=i.description)||void 0===m?void 0:m.map(i=>k(i)),k("this file is autogenerated. edit ./bin/MAGIC-doc-template.js to update"),P("@magic/mime-types"),f({id:"install"},"installation"),k("be in a nodejs project"),R(`
npm install @magic/mime-types
`),f({id:"usage"},"import / usage"),R({lines:!1,content:`
...
`}),x({id:"source"},"source"),k(["the source for this page is in the ",W({to:"https://github.com/magic/mime-types/tree/master/docsrc",text:"docsrc directory"})," and gets built and published to github using ",W({to:"https://github.com/magic/core",text:"@magic/core"})])]},"/mime-types/404/":()=>c("404 - not found.")};p({init:m(i({},{description:["mime-db mimetypes that have file extensions.","exports a key => value store, where key = extension and value = mime/type"],logotext:"@magic/mime-types",menu:[{text:"install",to:"/mime-types/#install"},{text:"usage",to:"/mime-types/#usage"},{text:"source",to:"/mime-types/#source"}],nospy:{show:!1},pageClass:{},pages:{"/mime-types/404/":{description:"404 - not found.",title:"404 - not found"}},root:"/mime-types/",theme:"dark",title:"@magic/mime-types",url:"/mime-types/"}),{url:window.location.pathname,hash:window.location.hash.substr(1)}),subscriptions:i=>[[G.listenPopState,I.pop]],view:i=>{let m=_[i.url]?i.url:"/404/",e=_[m],a=i.pages&&i.pages[m];return a&&Object.keys(a).forEach(m=>{i[m]=a[m]}),i.url=m,D({page:e,state:i},[N(i),L(i)])},node:document.getElementById("Magic")})})();